...
bool bufferedFindSpace(uint16_t targetLen, bool fromStart = true)
{
  ...
  while (f.position() < f.size() - 1)
  {
    ...
    while (count < bufsize)
    {
      ...
      {
        if (++index >= targetLen)
        { // return true if space long enough
          if (fromStart)
          {
      ...
      else
      {
        ...
        if (index)
        {
          if (knownLargestSpace < index || knownLargestSpace == UINT16_MAX)
...
bool appendObjectToFile(const char *key, JsonDocument *content, uint32_t s, uint32_t contentLen = 0)
{
  ...
  {
    if (f.position() > 2)
      f.write(','); // add comma if not first object
    f.print(key);
  ...
  if (f.size() + 9000 > (fsBytesTotal - fsBytesUsed))
  { // make sure there is enough space to at least copy the file once
  ...
  { // file content is not valid JSON object
    f.seek(0, SeekSet);
    f.print('{'); // start JSON
  }
...
bool writeObjectToFile(const char *file, const char *key, JsonDocument *content)
{
  ...
  if (!f && !HYDROPONICS_FS.exists(file))
    f = HYDROPONICS_FS.open(file, "w+");
  if (!f)
  {
...
void updateFSInfo()
{
#if HYDROPONICS_FS == LITTLEFS || ESP_IDF_VERSION_MAJOR >= 4
  fsBytesTotal = HYDROPONICS_FS.totalBytes();
  fsBytesUsed = HYDROPONICS_FS.usedBytes();
#else
  esp_spiffs_info(nullptr, &fsBytesTotal, &fsBytesUsed);
#endif
  DEBUG_PRINTF("FS total: %u, FS used: %u\n", fsBytesTotal, fsBytesUsed);
}
...
bool handleFileRead(AsyncWebServerRequest *request, String path)
{
  ...
  if (path.endsWith("/"))
    path += "index.htm";
  if (path.indexOf("sec") > -1)
    return false;
  String contentType = getContentType(request, path);
  /*String pathWithGz = path + ".gz";
  if(HYDROPONICS_FS.exists(pathWithGz)){
    request->send(HYDROPONICS_FS, pathWithGz, contentType);
    return true;
  }*/
  if (HYDROPONICS_FS.exists(path))
  {
    request->send(HYDROPONICS_FS, path, contentType);
    return true;
  }
  else
  {
    request->send(404, "application/javascript", "Not found");
    DEBUG_PRINTF("Requested file %s does not exist\n", path);
    return false;
  }
}
